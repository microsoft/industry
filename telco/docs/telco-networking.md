...
- For Azure regions that do support availability zones, deploy Azure ExpressRoute gateway as zone-redundant as this will provide the maximum availability within the region with gateway instances spread across multiple availability zones.
  - With this deployment, however, the telco solutions deployed across availability zones may experience extra latency, and also, this would incur in extra costs beginning from July 1, 2022.
...
- In Azure, connect virtual networks by using VNet peering when resources across different virtual networks need to communicate with each other.
  - A typical scenario is a management plane VNet communicating with data plane VNet(s) as depicted in figure 5 below:

![Figure 5: VNet peering between control plane and data plane VNets](./telco-multiple-er-vnet-peering.png)
_Figure 5:VNet peering between control plane and data plane VNets._
