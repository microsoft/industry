# Azure for Retail User Guide

This user guide explains the Azure for Microsoft Cloud for Retail (MCR) reference implementation, what it is, what it does, how retailers can use it to run their workloads on a sustainable, scalable, secure, and compliant Azure architecture.
...
## What is Azure for Retail reference implementation

The Azure for Retail industry reference implementation is an optimized, proven, authoritative, and roadmap aligned architecture that enables the Retail industry to deploy highly regulated retail workloads in Azure at scale.

Azure for Retail industry reference architecture provides prescriptive guidance coupled with Azure best practices for the retail industry, and it follows 5 design principles across the 8 critical design areas for organizations to define their target state for their Azure architecture.

The reference architecture is modular by design and allows organizations of any size in the retail industry to start with the optimized landing zones that support their workloads, and application portfolios.
...
## What if I already have existing Azure footprint

Azure for Retail industry reference implementation will meet the organizations in the retail industry where they are, and the design has catered for existing subscriptions and workloads on Azure.

See the following [article](https://docs.microsoft.com/en-us/azure/cloud-adoption-framework/ready/enterprise-scale/transition) to learn more how you can transition into an Azure architecture based on a multi-subscription design with clear separation of platform and landing zones, such as Azure for Retail.

## How Azure for Retail reference implementation works

This section describes at a high level how Azure for Retail reference implementation works. Your retail landing zones are the output of a multi-subscription environment for all your Azure services, where compliance, guardrails, security, networking, and identity is provided at scale by the platform.

## Azure for Retail design principles

The Azure for Retail architecture prescribed in this guidance is based on the design principles described here. These principles serve as a compass for subsequent design decisions across critical technical domains. Familiarize yourself with these principles to better understand their impact and the trade-offs associated with nonadherence, and how them will help you to scale in alignment with the Azure product roadmap.

>Note: The design principles for Azure are by design are industry agnostic and archetype neutral. For more details about architecture and design methodologies for Microsoft Clouds, see [this article](../../foundations/README.md).
...
## Azure for Retail Management Group Structure

The Management Group structure implemented with Azure for Retail is as follows:
...
- **Landing Zones:** This is the parent Management Group for all the landing zone subscriptions and will have workload agnostic Azure Policies assigned to ensure workloads are secure and compliant.
  ...
  - **Retail** This is the dedicated Management Group for retail landing zones for workloads subject to regulatory compliance. For example - some retailers host their own payment card switching platform internally.
...
## What happens when you deploy Azure for Retail reference implementation?
...
This section will describe how to deploy Azure for Retail industry reference implementation with traditional "hub and spoke" networking topology in Azure connected to the Distributed Edge and to on-premises datacenters and branch offices.

### Pre-requisites

Azure for Retail is a complete, end-to-end setup of your Azure tenant as a whole, the user making the deployment requires the "Owner" permission at the Azure tenant root scope. The following instructions explains how a Global Admin in the Azure Active Directory can elevate himself/herself - or others to have the required permissions prior to starting the deployment.
...
- A user that is Global Admin in the Azure Active Directory where Azure for Retail will be deployed.
...
The deployment experience in Azure portal allows you to bring in existing (preferably empty) subscriptions dedicated for the platform part of the Azure for Retail architecture. It also allows you to bring existing subscriptions that can be used as the initial retail landing zones for your retail workloads.

## Step by step guidance

This section will explain the deployment experience and the options provided for Azure for Retail reference implementation.
...
- DDoS Protection Standard to be enabled for the Azure platform. Enabling this will provide an option to assign requisite Azure Policy on the *retail landing zones* tab later.
...
If you don't need - or plan to host domain controllers in Azure for your Retail workloads, you can select *No*. If you later want to add dedicated subscription for these purposes, you can move it manually to the 'identity' management group in the hierarchy created by Azure for Retail.

 ![img](./media/auth.PNG)

### retail Landing Zones

On the *retail Landing Zones* tab, you can bring in the subscriptions you want to use initially for *retail*, *online*, and *corp* landing zones. Each landing zone type is represented by its own child management group of the *landing zones* management group in the hierarchy, and provides different characteristics regarding networking requirements, security, policy, and availability. For each of the landing zone types, you can select the recommended Azure policies you want to assign, as well as the policies recommended for *all* landing zone types.

- Retail landing zones

Subscriptions can be moved into the *retail* management group, and bootstrapped with Azure policies that are assigned directly to this management group, as well as the overarching policies assigned to the intermediate root management group, and the landing zones management group.
The specific policies for the retail landing zones will conform to the retail industry regulatory compliance.

![retaillz](./media/retaillz.PNG)
